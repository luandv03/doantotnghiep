...
class HarmonySearch:
    def preprocess_data(self):
        ...
        # Tạo dict tiện tra cứu nhanh (giống như trong Greedy)
        self.workers_dict = {w.id: w for w in self.workers}
        self.machines_dict = {m.asset_id: m for m in self.machines}
        
        # Cache cho việc tìm kiếm production order
        self.order_lookup = {order.order_id: order for order in self.production_orders}
    ...
    def generate_random_solution(self) -> tuple[Dict, List]:
        """
        Tạo một giải pháp ngẫu nhiên với cải tiến giảm thiểu tình trạng nhân viên bị khóa.
        ...
        # Bước 1: Phân tích dependencies để xác định thứ tự ưu tiên
        operation_priority = self.calculate_operation_priority()
        
        # Bước 2: Phân bổ tài nguyên theo thứ tự ưu tiên, tránh khóa tài nguyên
        available_workers = set(self.workers)
        available_machines = set(self.machines)
        
        # Theo dõi tài nguyên đã được phân bổ cho mỗi công đoạn
        allocated_workers = {}
        allocated_machines = {}
        
        for priority, operation in operation_priority:
            op_id = operation.operation_id
            
            # Tìm tài nguyên phù hợp cho công đoạn này
            eligible_workers = [w for w in available_workers if w.position == operation.required_position]
            eligible_machines = [m for m in available_machines if m.asset_type == operation.required_machine_type]
            
            # Tính số lượng tài nguyên cần thiết dựa trên KPI
            kpi_target = operation.assigned_kpis[0]["value"] if operation.assigned_kpis else 100
            estimated_workers_needed = max(1, min(3, len(eligible_workers)))
            estimated_machines_needed = max(1, min(3, len(eligible_machines)))
            
            # Chọn tài nguyên tốt nhất có sẵn
            selected_workers = []
            selected_machines = []
            
            # Chọn workers (ưu tiên năng suất cao)
            if eligible_workers:
                eligible_workers.sort(key=lambda w: w.productivity_kpi * w.quality_kpi, reverse=True)
                num_workers = min(estimated_workers_needed, len(eligible_workers))
                selected_workers = eligible_workers[:num_workers]
                
                # Chỉ loại bỏ workers khỏi available nếu công đoạn này có thể bắt đầu ngay
                if self.can_operation_start_immediately(operation):
                    for worker in selected_workers:
                        available_workers.discard(worker)
                else:
                    # Nếu công đoạn chưa thể bắt đầu, chỉ phân bổ tạm thời
                    allocated_workers[op_id] = selected_workers
            
            # Chọn machines (ưu tiên năng suất cao)
            if eligible_machines:
                eligible_machines.sort(key=lambda m: m.productivity, reverse=True)
                num_machines = min(estimated_machines_needed, len(eligible_machines))
                selected_machines = eligible_machines[:num_machines]
                
                # Chỉ loại bỏ machines khỏi available nếu công đoạn này có thể bắt đầu ngay
                if self.can_operation_start_immediately(operation):
                    for machine in selected_machines:
                        available_machines.discard(machine)
                else:
                    # Nếu công đoạn chưa thể bắt đầu, chỉ phân bổ tạm thời
                    allocated_machines[op_id] = selected_machines
            
            solution[op_id]["workers"] = selected_workers
            solution[op_id]["machines"] = selected_machines
        
        # Bước 3: Tái phân bổ tài nguyên bị khóa cho các công đoạn có thể bắt đầu ngay
        self.reallocate_blocked_resources(solution, allocated_workers, allocated_machines, available_workers, available_machines)
        
        # Bước 4: Phân bổ tài nguyên còn lại một cách linh hoạt
        self.allocate_remaining_resources(solution, available_workers, available_machines)
    ...
    def calculate_operation_priority(self):
        """
        Tính toán độ ưu tiên của các công đoạn dựa trên dependencies và khả năng bắt đầu.
        """
        operation_priority = []
        
        for operation in self.operations:
            # Tính độ ưu tiên dựa trên dependencies
            dependency_count = len(operation.prev_operation) if operation.prev_operation else 0
            dependent_count = sum(1 for deps in self.operation_dependencies.values() if operation.operation_id in deps)
            
            # Tính độ ưu tiên dựa trên KPI
            kpi_value = operation.assigned_kpis[0]["value"] if operation.assigned_kpis else 0
            
            # Tính khả năng bắt đầu ngay
            can_start_immediately = self.can_operation_start_immediately(operation)
            
            # Độ ưu tiên: công đoạn có thể bắt đầu ngay, ít dependencies, nhiều dependents, KPI cao
            priority = (can_start_immediately * 10) + dependency_count - 0.5 * dependent_count + 0.1 * kpi_value
            
            operation_priority.append((priority, operation))
        
        # Sắp xếp theo độ ưu tiên giảm dần
        operation_priority.sort(key=lambda x: x[0], reverse=True)
        
        return operation_priority
    
    def can_operation_start_immediately(self, operation):
        """
        Kiểm tra xem công đoạn có thể bắt đầu ngay lập tức không.
        """
        # Công đoạn có thể bắt đầu ngay nếu không có dependencies
        if not operation.prev_operation:
            return True
        
        # Hoặc nếu tất cả dependencies đã được hoàn thành (trong trường hợp này, chúng ta giả định chưa có công đoạn nào hoàn thành)
        return False
    
    def reallocate_blocked_resources(self, solution, allocated_workers, allocated_machines, available_workers, available_machines):
        """
        Tái phân bổ tài nguyên bị khóa cho các công đoạn có thể bắt đầu ngay.
        """
        # Tìm các công đoạn có thể bắt đầu ngay
        immediate_operations = [op for op in self.operations if self.can_operation_start_immediately(op)]
        
        # Tái phân bổ workers bị khóa
        for op_id, workers in allocated_workers.items():
            operation = next(op for op in self.operations if op.operation_id == op_id)
            
            # Nếu công đoạn này chưa thể bắt đầu, tái phân bổ workers cho công đoạn có thể bắt đầu
            if not self.can_operation_start_immediately(operation):
                for worker in workers:
                    # Tìm công đoạn có thể bắt đầu ngay và cần worker này
                    for immediate_op in immediate_operations:
                        if (immediate_op.required_position == worker.position and 
                            len(solution[immediate_op.operation_id]["workers"]) < 3):
                            solution[immediate_op.operation_id]["workers"].append(worker)
                            break
                    else:
                        # Nếu không tìm thấy, trả về available
                        available_workers.add(worker)
        
        # Tái phân bổ machines bị khóa
        for op_id, machines in allocated_machines.items():
            operation = next(op for op in self.operations if op.operation_id == op_id)
            
            # Nếu công đoạn này chưa thể bắt đầu, tái phân bổ machines cho công đoạn có thể bắt đầu
            if not self.can_operation_start_immediately(operation):
                for machine in machines:
                    # Tìm công đoạn có thể bắt đầu ngay và cần machine này
                    for immediate_op in immediate_operations:
                        if (immediate_op.required_machine_type == machine.asset_type and 
                            len(solution[immediate_op.operation_id]["machines"]) < 3):
                            solution[immediate_op.operation_id]["machines"].append(machine)
                            break
                    else:
                        # Nếu không tìm thấy, trả về available
                        available_machines.add(machine)
    
    def allocate_remaining_resources(self, solution, available_workers, available_machines):
        """
        Phân bổ tài nguyên còn lại một cách linh hoạt.
        """
        # Phân bổ workers còn lại
        for worker in available_workers:
            eligible_operations = [op for op in self.operations if op.required_position == worker.position]
            
            if eligible_operations:
                # Ưu tiên công đoạn có thể bắt đầu ngay và có ít workers
                eligible_operations.sort(key=lambda op: (
                    self.can_operation_start_immediately(op),
                    -len(solution[op.operation_id]["workers"])
                ), reverse=True)
                
                target_operation = eligible_operations[0]
                if len(solution[target_operation.operation_id]["workers"]) < 3:
                    solution[target_operation.operation_id]["workers"].append(worker)
        
        # Phân bổ machines còn lại
        for machine in available_machines:
            eligible_operations = [op for op in self.operations if op.required_machine_type == machine.asset_type]
            
            if eligible_operations:
                # Ưu tiên công đoạn có thể bắt đầu ngay và có ít machines
                eligible_operations.sort(key=lambda op: (
                    self.can_operation_start_immediately(op),
                    -len(solution[op.operation_id]["machines"])
                ), reverse=True)
                
                target_operation = eligible_operations[0]
                if len(solution[target_operation.operation_id]["machines"]) < 3:
                    solution[target_operation.operation_id]["machines"].append(machine)
    ...
    def hybridize_solutions(self, solution1, solution2):
        ...
        for operation_id in solution1:
            ...
            if not hybrid_solution[operation_id]["workers"]:
                eligible_workers = self.workers_by_position.get(operation.required_position, [])
                if eligible_workers:
            ...
            if not hybrid_solution[operation_id]["machines"]:
                eligible_machines = self.machines_by_type.get(operation.required_machine_type, [])
                if eligible_machines:
    ...
    def improvise_new_solution(self) -> tuple[Dict, List]:
        ...
        for op in self.operations:
            ...
            priority = dependency_count - 0.5 * dependent_count
            
            operations_priority.append((priority, op))
        
        # Sắp xếp các công đoạn theo độ ưu tiên tăng dần
        operations_priority.sort(key=lambda x: x[0])

        # Bước 1: Áp dụng Harmony Consideration cho các công đoạn theo thứ tự ưu tiên
        for _, operation in operations_priority:
                    ...
                    for worker in selected_solution[operation_id]["workers"]:
                        if worker not in used_workers and worker.position == operation.required_position:
                            ...
                            if random.random() < self.pitch_adjustment_rate:
                                ...
                                alternative_ops = [
                                    op for _, op in operations_priority
        ...
        for worker in unassigned_workers:
            ...
            if eligible_operations:
                ...
                for op in eligible_operations:
                    ...
                    # Trọng số theo độ ưu tiên của công đoạn (đơn giản hóa)
                    priority_idx = next(i for i, (p, o) in enumerate(operations_priority) if o.operation_id == op.operation_id)
                    priority_weight = math.exp(-priority_idx)
    ...
    def schedule_operations(
        ...
        # NHÁNH CẬN: Theo dõi số lệnh hoàn thành đúng hạn tối đa có thể đạt được
        max_possible_completed_orders = len(self.production_orders)  # Ban đầu = tổng số lệnh
        
        # Lấy trường hợp tồi nhất trong harmony memory để so sánh
        worst_completed_orders = 0
        worst_shifts = float('inf')
        if hasattr(self, 'harmony_memory') and self.harmony_memory:
            worst_fitness = max(self.harmony_memory, key=lambda x: (-x[1][0], x[1][1], x[1][2]))[1]
            worst_completed_orders = worst_fitness[0]
            worst_shifts = worst_fitness[1]
        ...
        def update_ready_operations():
            ...
            for op in operations:
                ...
                if all(prev_op in completed_operations for prev_op in op.prev_operation):
                    ...
                    order = self.order_lookup.get(op.production_order_id)
                    days_to_deadline = 0
                    if order:
            ...
            for shift in range(1, 5):  # Duyệt qua các ca từ 1 đến 4
                ...
                for _, operation in ready_operations:
                    ...
                    if is_completed:
                        ...
                        order = self.order_lookup.get(operation.production_order_id)
                        if (
                            order
                            ...
                            if current_date <= order_end_date:
                                ...
                                # NHÁNH CẬN: Lệnh hoàn thành không đúng hạn, giảm max_possible_completed_orders
                                max_possible_completed_orders -= 1
                                
                                # Kiểm tra nhánh cận: nếu max_possible_completed_orders < worst_completed_orders thì cắt nhánh
                                if max_possible_completed_orders < worst_completed_orders:
                ...
                # NHÁNH CẬN: Kiểm tra sau mỗi ca
                if max_possible_completed_orders < worst_completed_orders:
                    return completed_orders_on_time, total_shift, total_cost
                
                # NHÁNH CẬN: Kiểm tra về số ca làm việc
                if (max_possible_completed_orders <= worst_completed_orders and 
                    total_shift > worst_shifts):
                    return completed_orders_on_time, total_shift, total_cost
